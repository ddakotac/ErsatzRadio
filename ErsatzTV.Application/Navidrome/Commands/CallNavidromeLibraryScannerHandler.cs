...
public class CallNavidromeLibraryScannerHandler : CallLibraryScannerHandler<ISynchronizeNavidromeLibraryById>,
    IRequestHandler<ForceSynchronizeNavidromeLibraryById, Either<BaseError, string>>,
    IRequestHandler<SynchronizeNavidromeLibraryByIdIfNeeded, Either<BaseError, string>>
{
    private readonly IScannerProxyService _scannerProxyService;

    public CallNavidromeLibraryScannerHandler(
        IDbContextFactory<TvContext> dbContextFactory,
        IConfigElementRepository configElementRepository,
        IScannerProxyService scannerProxyService,
        IRuntimeInfo runtimeInfo,
        ILogger<CallNavidromeLibraryScannerHandler> logger)
        : base(dbContextFactory, configElementRepository, runtimeInfo, logger)
    {
        _scannerProxyService = scannerProxyService;
    }

    Task<Either<BaseError, string>> IRequestHandler<ForceSynchronizeNavidromeLibraryById, Either<BaseError, string>>.
        Handle(
            ForceSynchronizeNavidromeLibraryById request,
            CancellationToken cancellationToken) => Handle(request, cancellationToken);

    Task<Either<BaseError, string>>
        IRequestHandler<SynchronizeNavidromeLibraryByIdIfNeeded, Either<BaseError, string>>.
        Handle(
            SynchronizeNavidromeLibraryByIdIfNeeded request,
            CancellationToken cancellationToken) => Handle(request, cancellationToken);
    ...
    private async Task<Either<BaseError, string>> PerformScan(
        ...
        CancellationToken cancellationToken)
    {
        Option<Guid> maybeScanId = _scannerProxyService.StartScan(request.NavidromeLibraryId);
        foreach (var scanId in maybeScanId)
        {
            try
            {
                ...
                {
                    "scan-navidrome",
                    request.NavidromeLibraryId.ToString(CultureInfo.InvariantCulture),
                    GetBaseUrl(scanId)
                };
        ...
        return BaseError.New($"Library {request.NavidromeLibraryId} is already scanning");
    }
    ...
    protected override async Task<Tuple<string, DateTimeOffset>> GetLastScan(
        ...
        CancellationToken cancellationToken)
    {
        Option<NavidromeLibrary> maybeLibrary = await dbContext.NavidromeLibraries
            .SelectOneAsync(l => l.Id, l => l.Id == request.NavidromeLibraryId, cancellationToken);
